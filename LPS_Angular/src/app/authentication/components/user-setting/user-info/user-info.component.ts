...
export class UserInfoComponent implements OnInit {
  ...
  constructor(private userService: UserAccountService, private errorService: ErrorService, private authService: AuthorizationService) {
    // this.userService.viewUserInfo().subscribe(data => {
    //   this.currentUser = data;
    // });
    // this.warningColor.background_color.
  ...
  ngOnInit() {
    ...
    this.userService.getRoleFilter(1, '').subscribe(data => {
      ...
      if (this.department.length > 0) {
        ...
        this.userService.getRoleFilter(2, this.department[0].ROLE_CODE).subscribe(subDept => {
          ...
          //   this.userService.getRoleFilter(3, this.subDepartment[1].ROLE_CODE).subscribe(unitLeader => {
    ...
    // $('.toggle-button-container').click(function(){
  ...
  chooseUnit(id, name) {
    ...
    // $('#user-info-main-unit-modal').modal('hide');
  }
